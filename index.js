...
async function checkOllama() {
  ...
  // GPU overhead check moved to Environment section for accuracy
}
...
async function checkGateway() {
  ...
  if (portCheck) {
    ...
    // Count unique PIDs — IPv4 + IPv6 on same PID is normal (1 process)
    const pids = [...new Set(listeners.map(l => l.trim().split(/\s+/).pop()))];
    if (pids.length > 1) {
      report('fail', 'gateway', `${pids.length} different processes on port 18789 — 409 conflicts! PIDs: ${pids.join(', ')}`, 'Kill extras with taskkill /PID <pid> /F');
...
async function checkEnv() {
  ...
  // Check OLLAMA_GPU_OVERHEAD in both process.env AND system registry (process.env can be stale)
  const fromEnv = process.env.OLLAMA_GPU_OVERHEAD;
  const fromReg = run('powershell -NoProfile -Command "[System.Environment]::GetEnvironmentVariable(\'OLLAMA_GPU_OVERHEAD\', \'User\')"')
    || run('powershell -NoProfile -Command "[System.Environment]::GetEnvironmentVariable(\'OLLAMA_GPU_OVERHEAD\', \'Machine\')"');
  const gpuOverhead = fromEnv || (fromReg && fromReg !== '' ? fromReg : null);
  if (gpuOverhead)
    report('fail', 'env', `OLLAMA_GPU_OVERHEAD="${gpuOverhead}" — forces CPU mode!`,
      'Remove: [System.Environment]::SetEnvironmentVariable("OLLAMA_GPU_OVERHEAD", $null, "User") AND "Machine"');
  else
