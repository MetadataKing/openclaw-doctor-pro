...
async function checkConfig() {
  ...
  for (const [name, provider] of Object.entries(providers)) {
    ...
    if (provider.api === 'ollama' && url.endsWith('/v1')) {
      ...
      if (FIX_MODE) {
        config.models.providers[name].baseUrl = url.replace('/v1', '');
        configChanged = true;
        reportFixed(`Removed /v1 from ${name} baseUrl → ${config.models.providers[name].baseUrl}`);
      }
    ...
    if (url.includes('0.0.0.0')) {
      ...
      if (FIX_MODE) {
        config.models.providers[name].baseUrl = url.replace('0.0.0.0', '127.0.0.1');
        configChanged = true;
        reportFixed(`Changed 0.0.0.0 → 127.0.0.1 for ${name}`);
      }
    }

    if (provider.api === 'ollama' && (!provider.models || provider.models.length === 0))
      report('warn', 'config', `Provider "${name}": no models defined`, 'Add models to config');
  ...
  if (primary) report('info', 'config', `Primary model: ${primary}`);
    ...
    report('fail', 'config', 'No primary model set', 'openclaw config set agents.defaults.model.primary "ollama/qwen2.5:7b"');
    if (FIX_MODE) {
      deepSet(config, 'agents.defaults.model.primary', 'ollama/qwen2.5:7b');
      configChanged = true;
      reportFixed('Set primary model → ollama/qwen2.5:7b');
    }
  }
  ...
  if (deny.length === 0) {
    report('warn', 'config', 'No tools denied — small models vulnerable to prompt injection');
    if (FIX_MODE) {
      if (!config.tools) config.tools = {};
      config.tools.deny = ['browser'];
      configChanged = true;
      reportFixed('Set tools.deny → ["browser"]');
    }
  } else {
    report('pass', 'config', `Tools deny list: ${deny.join(', ')}`);
  }
  ...
  if (telegram?.botToken) {
    ...
    if (!telegram.dmPolicy || telegram.dmPolicy === 'open') {
      ...
      if (FIX_MODE) {
        ...
        configChanged = true;
        reportFixed('Set Telegram DM policy → "pairing"');
      }
    }
  }

  // FIX: weak gateway token
  const gw = config?.gateway;
  if (gw?.auth?.token === 'ollama' || gw?.auth?.token === 'test') {
    report('warn', 'config', `Gateway auth token "${gw.auth.token}" is weak`, 'Use a strong random token');
    if (FIX_MODE) {
      const strong = Array.from({ length: 32 }, () => Math.random().toString(36)[2]).join('');
      config.gateway.auth.token = strong;
      configChanged = true;
      reportFixed(`Generated strong gateway token: ${strong.slice(0, 8)}...`);
    }
  }
  ...
  if (ps) {
    ...
    lines.forEach(line => {
      if (line.includes('100% CPU')) {
        ...
        report('fail', 'ollama', `Model "${model}" on CPU — extremely slow!`, 'Remove OLLAMA_GPU_OVERHEAD, restart Ollama');
        if (FIX_MODE) {
          // Try to restart Ollama to reload on GPU
          run('powershell -NoProfile -Command "Get-Service *ollama* | Restart-Service -Force"');
          reportFixed('Restarted Ollama service — model should reload on GPU');
        }
    ...
    if (lines.length === 0) {
      report('warn', 'ollama', 'No models loaded', 'ollama run qwen2.5:7b --keepalive 24h');
      if (FIX_MODE) {
        console.log(`  ${c.cyan('⏳')}  Loading primary model (this may take a moment)...`);
        run('ollama run qwen2.5:7b --keepalive 24h "hello"');
        reportFixed('Loaded qwen2.5:7b with 24h keepalive');
      }
    }
  }
}
...
async function checkGateway() {
  ...
  if (portCheck) {
    ...
    if (pids.length > 1) {
      report('fail', 'gateway', `${pids.length} processes on port 18789 — 409 conflicts! PIDs: ${pids.join(', ')}`, 'Kill extras');
      if (FIX_MODE) {
        // Keep first PID, kill the rest
        const toKill = pids.slice(1);
        toKill.forEach(pid => {
          run(`taskkill /PID ${pid} /F`);
          reportFixed(`Killed duplicate gateway process PID ${pid}`);
        });
      }
    } else if (pids.length === 1) {
      report('pass', 'gateway', `Gateway active on port 18789 (PID ${pids[0]})`);
    }
  } else {
...
async function checkTelegram(config) {
  ...
  try {
    ...
    if (wh.result?.url && wh.result.url !== '') {
      report('fail', 'telegram', 'Webhook active — conflicts with polling!', 'Delete webhook');
      if (FIX_MODE) {
        run(`curl -s "https://api.telegram.org/bot${token}/deleteWebhook?drop_pending_updates=true"`);
        reportFixed('Deleted Telegram webhook');
      }
    } else {
      report('pass', 'telegram', 'No webhook (good)');
    }
  } catch {}
}
...
async function checkDocker(config) {
  ...
  if (!ver) { report('fail', 'docker', 'Docker not installed but sandbox is "all"', 'Install Docker Desktop or set sandbox to "off"'); return; }
...
async function checkEnv() {
  ...
  // Check OLLAMA_GPU_OVERHEAD in process.env AND system registry
  ...
  if (gpuOverhead) {
    report('fail', 'env', `OLLAMA_GPU_OVERHEAD="${gpuOverhead}" — forces CPU mode!`,
      'Remove from User AND Machine env vars, restart Ollama');
    if (FIX_MODE) {
      if (hasUser) {
        run('powershell -NoProfile -Command "[System.Environment]::SetEnvironmentVariable(\'OLLAMA_GPU_OVERHEAD\', $null, \'User\')"');
        reportFixed('Removed OLLAMA_GPU_OVERHEAD from User environment');
      }
      if (hasMachine) {
        run('powershell -NoProfile -Command "Start-Process powershell -Verb RunAs -ArgumentList \'-NoProfile -Command [System.Environment]::SetEnvironmentVariable(\\\"OLLAMA_GPU_OVERHEAD\\\", $null, \\\"Machine\\\")\'"');
        reportFixed('Removed OLLAMA_GPU_OVERHEAD from Machine environment (admin prompt)');
      }
      // Restart Ollama to pick up env change
      run('powershell -NoProfile -Command "Get-Service *ollama* | Restart-Service -Force"');
      reportFixed('Restarted Ollama service');
    }
  } else {
    report('pass', 'env', 'No OLLAMA_GPU_OVERHEAD (good)');
  }
}
...
async function main() {
  ...
  if (FIX_MODE) {
    ...
    console.log(c.dim('Run with --fix to auto-repair issues\n'));
  }
